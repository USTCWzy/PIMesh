...
class Renderer(object):
    ...
    def __init__(self, focal_length=600, img_w=3000, img_h=3000, faces=None,
    ...
    def render_front_view(self, verts, bg_img_rgb=None, bg_color=(0, 0, 0, 0), angle=None, axis=None):
        ...
        if angle and axis:
            ...
            # bg_img_rgb = np.zeros_like(bg_img_rgb)


        if self.same_mesh_color:
            mesh_color = colorsys.hsv_to_rgb(0.5, 0.5, 0.5)
            # mesh_color = (0.5, 0.5, 0.5)
        else:
            mesh_color = colorsys.hsv_to_rgb(1.0, 0.5, 1.0)
        ...
        if bg_img_rgb is None:
            img = np.ones_like(color_rgb, dtype=np.uint8) * 127
            mask = depth_map > 0
            img[mask] = color_rgb[mask]
            return img
        else:
            mask = depth_map > 0
    ...
    def render_side_view(self, verts):
        ...
        side_view = self.render_front_view(pred_vert_arr_side[0])
        return side_view
    def delete(self):
        """
