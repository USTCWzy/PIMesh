...
from model.ViT.layers.drop import DropPath
from model.ViT.layers.mlp import Mlp
from model.ViT.layers.weight_init import trunc_normal_
from model.ViT.layers.patch_embed import PatchEmbed, PositionEmbed
from model.ViT.utils.mask_embedding import MaskEmbedding, UnMaskEmbedding
...
#         :param x:
#             B: batch size
#             N: max squence length
#             C: d_model (dim of word embedding)
#         :return:
#         '''
#         B, N, C = x.shape
...
#             q, k, v = qkv[0].float(), qkv[1].float(), qkv[
#                 2].float()  # make torchscript happy (cannot use tensor as tuple)
#             attn = (q @ k.transpose(-2, -1)) * self.scale
#             attn = attn.softmax(dim=-1)
#             attn = self.attn_drop(attn)
#             x = (attn @ v).transpose(1, 2).reshape(B, N, C)
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = qkv.unbind(0)
        qn, kn = self.q_norm(q), self.k_norm(k)

        qn = qn * self.scale

        attn = qn @ kn.transpose(-2, -1)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if self.vit_type == "decoder":
            # import pdb;pdb.set_trace()
            self.unmask_embed = UnMaskEmbedding(img_size, embed_dim, in_chans, patch_size, num_patches)
        ...
        if isinstance(module, nn.Linear):
            if module.out_features == self.num_classes:
                ...
                if module.bias is not None:
                    ...
                    trunc_normal_(module.weight, dtf=.02)
                    if module.bias is not None:
                        nn.init.zeros_(module.bias)
    ...
    def autoencoder(self, x):
        ...
        dumpy_token = self.cls_token.expand(x.shape[0], -1, -1)
        x = torch.cat((dumpy_token, x), dim=1)
        x = x + self.pos_embed

        # mask the patch & pose_emd
        mask_patch_embedding, sample_index, mask_index = MaskEmbedding(x, self.mask_ratio)
    ...
    def decoder(self, x, sample_index, mask_index):
        '''
        decode the all patch embedding with the mask and postition embedding
        '''
        # import pdb;pdb.set_trace()
        decoder_embed = self.unmask_embed(x, sample_index, mask_index)
        x = decoder_embed + self.pos_embed

        decoder_embedding = self.blocks(x)
        return decoder_embedding

    def forward_features(self, x):
        '''
        return the layer_norm features
        '''
        if torch.isnan(x).any():
            import pdb;pdb.set_trace()
        x = self.patch_embed(x)
    ...
    def forward(self, x):
        ...
        assert x.shape[1] == self.num_features, 'outputs must be the same with the features'
        if self.classification:
            x = self.class_head(x)
        return x
